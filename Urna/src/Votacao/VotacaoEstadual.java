...
public class VotacaoEstadual extends JFrame{
	...
	public VotacaoEstadual(List<Candidato>estadual){
		super("Votação Deputado Estadual");
		listacandidados = estadual;
		...
		candidato.setText("DEPUTADO(A) ESTADUAL:");
	...
	public class Manipulador implements ActionListener{
		public void actionPerformed(ActionEvent evento){
			...
			if (evento.getSource() == branco) {
				dispose();
				new TelaFinal(listacandidados);
			}
			if (evento.getSource() == confirma) {
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.DeputadoEstadual") == true && Integer.parseInt(numero.getText())==listacandidados.get(i).getNumero()) {
			...
			if (evento.getSource() == lista) {
				String tudo ="CANDIDATOS A DEPUTADO ESTADUAL\n";
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.DeputadoEstadual")==true) {
...
class TelaFinal extends JFrame{
		JButton votar, resultado;
		
		public TelaFinal(List<Candidato> listacandidados){
			...
			votar = new JButton();
			votar.setText("VOTAR NOVAMENTE");
			votar.addActionListener((ActionListener) objeto);
			votar.setBounds(40, 70, 200, 40);
			tela.add(votar);
			
			resultado = new JButton();
			resultado.setText("RESULTADO DAS ELEIÇÕES");
			resultado.addActionListener(objeto);
			resultado.setBounds(40, 170, 200, 40);
			tela.add(resultado);
		...
		public class Manipulador implements ActionListener{
			public void actionPerformed(ActionEvent evento){
				if(evento.getSource() == votar){
					dispose();
					new VotacaoPresidente(listacandidados);
				}
				if (evento.getSource() == resultado) {
					dispose();
					new Resultado(listacandidados);				
				}
			}
		}
	}
