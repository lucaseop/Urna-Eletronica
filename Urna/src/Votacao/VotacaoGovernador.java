...
public class VotacaoGovernador extends JFrame{
	...
	public VotacaoGovernador(List<Candidato>governador){
		super("Votação Governador");
		listacandidados = governador;
		...
		candidato.setText("GOVERNADOR(A):");
	...
	public class Manipulador implements ActionListener{
		public void actionPerformed(ActionEvent evento){
			...
			if (evento.getSource() == branco) {
				new VotacaoSenador(listacandidados);
				dispose();
			}
			if (evento.getSource() == confirma) {
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.Governador") == true && Integer.parseInt(numero.getText())==listacandidados.get(i).getNumero()) {
			...
			if (evento.getSource() == lista) {
				String tudo ="CANDIDATOS A GOVERNADOR\n";
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.Governador")==true) {
