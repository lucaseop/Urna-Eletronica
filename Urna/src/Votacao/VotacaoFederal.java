...
public class VotacaoFederal extends JFrame{
	...
	public VotacaoFederal(List<Candidato>federal){
		super("Votação Deputado Federal");
		listacandidados = federal;
		...
		candidato.setText("DEPUTADO(A) FEDERAL:");
	...
	public class Manipulador implements ActionListener{
		public void actionPerformed(ActionEvent evento){
			...
			if (evento.getSource() == branco) {
				new VotacaoEstadual(listacandidados);
				dispose();
			}
			if (evento.getSource() == confirma) {
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.DeputadoFederal") == true && Integer.parseInt(numero.getText())==listacandidados.get(i).getNumero()) {
			...
			if (evento.getSource() == lista) {
				String tudo ="CANDIDATOS A DEPUTADO FEDERAL\n";
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.DeputadoFederal")==true) {
