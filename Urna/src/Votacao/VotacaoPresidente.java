...
public class VotacaoPresidente extends JFrame{
	...
	public VotacaoPresidente(List<Candidato>presidente){
		super("Votação Presidente");
		listacandidados = presidente;
		...
		candidato.setText("PRESIDENTE(A):");
	...
	public class Manipulador implements ActionListener{
		public void actionPerformed(ActionEvent evento){
			...
			if (evento.getSource() == branco) {
				new VotacaoGovernador(listacandidados);
				dispose();
			}
			if (evento.getSource() == confirma) {
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.Presidente") == true && Integer.parseInt(numero.getText())==listacandidados.get(i).getNumero()) {
			...
			if (evento.getSource() == lista) {
				String tudo ="CANDIDATOS A PRESIDENTE\n";
				for (int i = 0; i < listacandidados.size(); i++) {
					if (listacandidados.get(i).getClass().getName().equals("Candidatos.Presidente")==true) {
